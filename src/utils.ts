 ...
 */
export function parseColor(color: ColorInput): RGBAColor {
  ...
  const colorStr = (color as string).trim().toLowerCase();
 ...
 */
export function getDirectionAngle(direction: BurstDirection, customAngle?: number): number {
  ...
  const angle = DIRECTION_ANGLES[direction as keyof typeof DIRECTION_ANGLES];
 ...
 */
export function mergeConfig(options?: ConfettiBurstOptions): ConfettiBurstConfig {
  if (!options) {
  ...
  return {
    ...
    spawnArea: options.spawnArea ? { type: 'rect', x: 0, y: 0, ...options.spawnArea } as SpawnArea : undefined,
 ...
 */
export function getDevicePixelRatio(maxDpr: number = 2): number {

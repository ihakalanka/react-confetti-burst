...
exports.ConfettiEngine = void 0;
exports.createConfettiExplosion = createConfettiExplosion;
exports.fireFromElement = fireFromElement;
exports.getActiveAnimationCount = getActiveAnimationCount;
...
function getOrCreateCanvas(zIndex, canvasConfig) {
    ...
    const config = { ...constants_1.DEFAULT_CANVAS, ...canvasConfig };
    const dpr = config.pixelRatio ?? (0, utils_1.getDevicePixelRatio)(constants_1.PERFORMANCE.MAX_DPR);
    if (!globalCanvas) {
...
function getSpawnPoint(area, _canvasWidth, _canvasHeight) {
    ...
    switch (type) {
        ...
        case 'rect':
            return {
                x: (0, utils_1.randomInRange)(x, x + (w ?? 0)),
                y: (0, utils_1.randomInRange)(y, y + (h ?? 0)),
            };
        case 'line':
            const t = (0, utils_1.secureRandom)();
            return {
                x: x + t * (w ?? 0),
                y: y + t * (h ?? 0),
            };
        case 'circle':
            const angle = (0, utils_1.secureRandom)() * constants_1.MATH_CONSTANTS.TWO_PI;
            const radius = Math.sqrt((0, utils_1.secureRandom)()) * (w ?? 50); // sqrt for uniform distribution
            return {
...
class ConfettiEngine {
    constructor(origin, options) {
        ...
        this.isPaused = false;
        this.deferred = (0, utils_1.createDeferred)();
        ...
        this.animate = () => {
            ...
            for (const p of this.particles) {
                if (p.active) {
                    (0, particle_1.updateParticle)(p, deltaTime, physics, particle.fadeOut, particle.scaleDown);
                    ...
                    (0, particle_1.renderParticle)(ctx, p, { customDraw: this.drawShape });
                }
            }
            ...
            if (this.mode === 'continuous' || this.mode === 'snow' || this.mode === 'rain') {
                ...
                if (timeSinceLastSpawn >= spawnInterval) {
                    const activeCount = (0, particle_1.countActiveParticles)(this.particles);
            ...
            if (this.mode === 'burst' || this.mode === 'cannon' || this.mode === 'explosion') {
                if ((0, particle_1.areAllParticlesInactive)(this.particles)) {
                    this.complete();
                    return;
                }
            }
        ...
        this.origin = origin;
        this.config = (0, utils_1.mergeConfig)(options);
        // Set up mode-specific configuration
        this.mode = options?.mode ?? 'burst';
        this.spawnArea = options?.spawnArea;
        this.continuousConfig = { ...constants_1.DEFAULT_CONTINUOUS, ...options?.continuous };
        this.fireworkConfig = { ...constants_1.DEFAULT_FIREWORK, ...options?.firework };
        this.canvasConfig = { ...constants_1.DEFAULT_CANVAS, ...options?.canvas };
    ...
    createParticles() {
        ...
        const count = Math.min(particleCount, constants_1.PERFORMANCE.MAX_PARTICLES);
        for (let i = 0; i < count; i++) {
            // Calculate direction angle with spread
            let angle;
            if (direction.direction === 'radial') {
                // Full 360° spread for radial
                angle = (0, utils_1.secureRandom)() * constants_1.MATH_CONSTANTS.TWO_PI;
            }
            else {
                // Directional burst with spread
                const baseAngle = (0, utils_1.getDirectionAngle)(direction.direction, direction.angle);
                const spreadRad = (0, utils_1.degToRad)(direction.spread);
                angle = baseAngle + ((0, utils_1.secureRandom)() - 0.5) * spreadRad;
            }
            // Random velocity within range
            const velocity = (0, utils_1.randomInRange)(direction.velocity[0], direction.velocity[1]);
            const newParticle = (0, particle_1.createParticle)(x, y, angle, velocity, particle.colors, particle.shapes, particle.size, particle.opacity, particle.lifespan, physics.rotationSpeed, {
    ...
    spawnParticles(count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (this.spawnArea) {
                ...
                spawnPoint = { x: (0, utils_1.randomInRange)(0, width), y: -20 };
            }
            else {
                // Default to top center area
                spawnPoint = { x: (0, utils_1.randomInRange)(0, width), y: (0, utils_1.randomInRange)(-20, 0) };
            }
            // Calculate angle based on mode
            let angle;
            let velocity;
            if (this.mode === 'snow') {
                angle = (0, utils_1.degToRad)((0, utils_1.randomInRange)(-20, 20) - 90); // Mostly down with some drift
                velocity = (0, utils_1.randomInRange)(0.5, 2);
            }
            else if (this.mode === 'rain') {
                angle = (0, utils_1.degToRad)(-90); // Straight down
                velocity = (0, utils_1.randomInRange)(8, 15);
            }
            else {
                angle = (0, utils_1.secureRandom)() * constants_1.MATH_CONSTANTS.TWO_PI;
                velocity = (0, utils_1.randomInRange)(2, 6);
            }
            const newParticle = (0, particle_1.createParticle)(spawnPoint.x, spawnPoint.y, angle, velocity, particle.colors, particle.shapes, particle.size, particle.opacity, particle.lifespan, physics.rotationSpeed, {
                images: particle.images,
                tiltRange: particle.tilt,
                spinSpeedRange: particle.spinSpeed,
                depth3D: particle.depth3D,
    ...
    launchFirework() {
        ...
        const launchX = this.spawnArea
            ...
            : (0, utils_1.randomInRange)(width * 0.2, width * 0.8);
        // Target explosion point
        const targetY = height * (1 - launchHeight);
        const targetX = launchX + (0, utils_1.randomInRange)(-50, 50);
        // Create rocket particle (goes up, then triggers explosion)
        const rocketParticle = (0, particle_1.createParticle)(launchX, height, (0, utils_1.degToRad)(-90 + (0, utils_1.randomInRange)(-10, 10)), (0, utils_1.randomInRange)(15, 25), rocketColors, ['circle'], [3, 5], [1, 1], 2000, 0);
    ...
    explodeFirework(x, y, data) {
        ...
        const spreadRad = (0, utils_1.degToRad)(spread);
        for (let i = 0; i < burstCount; i++) {
            const angle = (i / burstCount) * spreadRad - spreadRad / 2 + (0, utils_1.degToRad)(-90);
            const velocity = (0, utils_1.randomInRange)(4, 12);
            const burstParticle = (0, particle_1.createParticle)(x, y, angle, velocity, colors, this.config.particle.shapes, [2, 6], [0.8, 1], 1500, this.config.physics.rotationSpeed);
            // Mark for secondary explosion
            if (secondaryExplosions && (0, utils_1.secureRandom)() < 0.1) {
     ...
     */
    getParticleCount() {
        return (0, particle_1.countActiveParticles)(this.particles);
    }
    /**
...
exports.ConfettiEngine = ConfettiEngine;
/**
 * Creates and starts a confetti explosion
 */
function createConfettiExplosion(origin, options) {
...
function fireFromElement(element, options) {
    if (!element || !(0, utils_1.isBrowser)()) {
        return null;
    }
